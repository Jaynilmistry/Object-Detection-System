...
class ObjectDetectionApp:
    ...
    def create_detection_tab(self):
        ...
        self.canvas = tk.Canvas(detection_frame, width=800, height=400, bg="#333333")
        ...
        self.style.configure("Custom.TButton", foreground="white", background="#0F3460")

        self.load_button = ttk.Button(button_frame, text="📂 Load Image", style="Custom.TButton", command=self.load_image)
        self.load_button.grid(row=0, column=0, padx=10, pady=10)

        self.detect_button = ttk.Button(button_frame, text="🧠 Detect Objects",  style="Custom.TButton", command=self.detect_objects, state=tk.DISABLED)
        self.detect_button.grid(row=0, column=1, padx=10, pady=10)

        self.webcam_button = ttk.Button(button_frame, text="🎥 Start Webcam", style="Custom.TButton", command=self.start_webcam)
        self.webcam_button.grid(row=0, column=2, padx=10, pady=10)

        self.stop_webcam_button = ttk.Button(button_frame, text="🛑 Stop Webcam", style="Custom.TButton", command=self.stop_webcam, state=tk.DISABLED)
        self.stop_webcam_button.grid(row=0, column=3, padx=10, pady=10)

        self.detected_label = ttk.Label(detection_frame, text="Detected Objects:", font=("Helvetica", 14, "bold"), foreground="#00FFFF", background="#1E1E1E")
        self.detected_label.pack(pady=10)

        self.detected_info = ttk.Label(detection_frame, text="", font=("Helvetica", 10), foreground="#FFFFFF", background="#1E1E1E")
